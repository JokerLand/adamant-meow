...
object Marcourt extends jacop {
  ...
  def main(args: Array[String]) {
    ...
    if (totalEctsReussis < 5) {
      ...
      val bloquantIooIa = IntVar("", 0, 1)
      (ioo._2 /\ ia._2) <=> (bloquantIooIa #= 1)
      
      val toutLesCoursBloquant = bloquantIooIa
      
      val transgression = toutLesPrerequis + toutLesCoursBloquant * 3
      ...
      val b = minimize(search(listeCoursIntVar, input_order, indomain_max), transgression);
      if (b)
        printSol(listeCoursIntVar);
      else
